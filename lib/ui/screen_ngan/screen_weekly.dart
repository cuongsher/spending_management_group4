...
class ReportWeeklyScreen extends ConsumerStatefulWidget {
  const ReportWeeklyScreen({super.key});

  @override
  ConsumerState<ReportWeeklyScreen> createState() => _ReportWeeklyScreenState();
}

// 2. Chuyển sang ConsumerState
class _ReportWeeklyScreenState extends ConsumerState<ReportWeeklyScreen> {
  late final ReportWeeklyController _controller;
  ...
  void initState() {
    ...
    _controller = ReportWeeklyController(repo);
    _controller.loadWeek(_anchorDay);
  }

  Future<void> _pickAnyDayInWeek() async {
  ...
  (DateTime, DateTime) _weekRange(DateTime anyDay) {
    final d = DateTime(anyDay.year, anyDay.month, anyDay.day);
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (_, __) {
        ...
        final range = _weekRange(_anchorDay);
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Expanded(
                                    child: Text(
                                      'Tuần: ${_fmt(range.$1)} → ${_fmt(range.$2)}',
